...
def agent_update(sensing_agent):
    ...
    if not sensing_agent.ALLOW_ROTATION:
        print(f"{sensing_agent._id} ROTATION DISABLED:\t{est_rotation}")
        est_rotation = None

    if not sensing_agent.ALLOW_TRANSLATION:
        print(f"{sensing_agent._id} TRANSLATION DISABLED:\t{est_translation}")
        est_translation = None
